import { Button, Card, CardBody, CardFooter, CardHeader, Divider, Listbox, ListboxItem, ListboxSection, Spinner } from "@heroui/react";
import { Link } from "react-router";

function AdminActionCard({ title, icon, link = "/", children, loading }) {

    return (
        <Link className="flex flex-col sm:grow basis-[45%] lg:basis-[30%]" to={link}>
            <Card
                className="min-h-28"
                shadow="sm"
                isPressable
            >
                ...
                <CardFooter className="text-foreground-700 text-sm flex flex-row justify-end">
                    <Button color="primary" variant="light">Presiona para editar</Button>
                </CardFooter>
            </Card>
        </Link>
    )
