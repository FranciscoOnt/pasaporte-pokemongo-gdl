...
export const RenderUserCell = ({ user, columnKey }) => {
  ...
  switch (columnKey) {
    ...
    case "collection":
      return (
        <Link to={`/admin/usuarios/${user.uuid}`} className="flex flex-row">
          <BsPassport size={24} />
          <span className="my-auto">Ver</span>{" "}
        </Link>
      );
    case "lastLogin":
      return (
        <p className="text-bold text-small capitalize">
          {displayDate(user.lastLogin)}
        </p>
      );
    case "createdAt":
      return (
    ...
    case "nameUpdateDate":
      return (
        <p className="text-bold text-small capitalize">
          {displayDate(user.nameUpdateDate)}
        </p>
      );
    case "teamUpdateDate":
      return (
        <p className="text-bold text-small capitalize">
          {displayDate(user.teamUpdateDate)}
        </p>
      );
    case "actions":
      return (
        <div className="relative flex justify-end items-center gap-2">
          <Dropdown>
            ...
            <DropdownMenu>
              <DropdownItem onPress={() => resetRequest(user)} key="reset-name">
                Reiniciar Cambio de Nombre
              </DropdownItem>
              <DropdownItem
                onPress={() => resetRequest(user, false)}
                key="reset-team"
              >
                Reiniciar Cambio de Equipo
              </DropdownItem>
            </DropdownMenu>
          </Dropdown>
        </div>
